...
type CarouselProps = {
    ...
    /** built-in indicators (you can turn these off and render your own) */
    ...
    /** lets parent mirror the active index (for custom indicators) */
    onIndexChange?: (index: number) => void;
};

const HeroCarousel = React.forwardRef<CarouselHandle, CarouselProps>(
    function HeroCarousel(
        {
            ...
            showArrows = false, // kept for later if you want
            className = "",
            slideClassName = "",
        ...
        const [index, setIndex] = React.useState(() =>
            clampIndex(Math.min(Math.max(initialIndex, 0), Math.max(count - 1, 0)))
        );
        ...
        React.useImperativeHandle(
            ...
            [goTo, next, prev, index]
        );

        // tell parent when index changes (for custom indicators)
        React.useEffect(() => {
            onIndexChange?.(index);
        }, [index, onIndexChange]);
